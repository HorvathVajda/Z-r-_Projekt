...
const db = require("../config/db"); // Itt biztos, hogy a mysql2 csomaggal van csatlakoztatva az adatbázis
...
exports.login = async (req, res) => {
  ...
  try {
    // A db.query() helyett db.execute() használata biztosítja a megfelelő működést
    const [users] = await db.execute("SELECT * FROM felhasznalo WHERE email = ?", [email]);
...
exports.registerUser = async (req, res) => {
  ...
  try {
    // Ellenőrizzük, hogy van-e már felhasználó ugyanazzal az e-mail címmel
    const [existingUsers] = await db.execute("SELECT * FROM felhasznalo WHERE email = ?", [email]);
